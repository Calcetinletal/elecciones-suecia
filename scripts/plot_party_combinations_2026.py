...
Run: .venv/bin/python scripts/plot_party_combinations_2026.py --combination S (or V, SV, SVG)
"""
...
parser = argparse.ArgumentParser(description='Independent 2026 district scatterplots')
parser.add_argument('--combination', choices=['S', 'V', 'SV', 'SVG'], required=True)
combination = parser.parse_args().combination
config = {
    'S': (['S'], 'S', 'S: Social Democrats'),
    'V': (['V'], 'V', 'V: Left Party'),
    'SV': (['S', 'V'], 'S + V', 'S: Social Democrats · V: Left Party'),
    'SVG': (['S', 'V', 'MP'], 'S + V + G', 'S: Social Democrats · V: Left Party · G: Green Party (MP)'),
}
party_ids, party_label, party_description = config[combination]
ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / 'public/data/2026/joined_2026.csv'
OUT = ROOT / 'reports/three-plots-2026'
OUT.mkdir(parents=True, exist_ok=True)
STEM = f'{combination}-2026-Calcetinletal-en-v5-yfit'
...
for row in source_rows:
    ...
    if row['birth_regions_complete'].lower() != 'true':
        ...
        continue
    combined_votes = sum({'S': votes_s, 'V': votes_v, 'MP': votes_mp}[party] for party in party_ids)
    assert 0 <= combined_votes <= valid and 0 <= born <= population
    x, y = 100 * born / population, 100 * combined_votes / valid
    ...
    assert abs(y - sum(float(row[f'pct_{party}']) for party in party_ids)) < 0.000003
    points.append({
...
y = np.array([r['selected_vote_pct'] for r in points])
...
ymax = min(100, max(5, math.ceil((float(y.max()) + 0.5) / 5) * 5))
...
summary = {
    ...
    'party_codes': party_ids,
    'y_definition': '100 * (' + ' + '.join('vote_' + party for party in party_ids) + ') / valid_votes',
...
(OUT / (STEM + '-statistics.json')).write_text(json.dumps(summary, ensure_ascii=False, indent=2, allow_nan=False), encoding='utf-8')
...
ax.yaxis.set_major_locator(MultipleLocator(10 if ymax <= 70 else 20))
...
ax.set_ylabel(f'{party_label} vote share (% of valid votes)', labelpad=15, fontsize=12)
fig.text(0.09, 0.946, 'SWEDEN  /  2026 ELECTION', color='#7c5269', size=10, weight='bold')
fig.text(0.09, 0.896, f'{party_label} and residents born outside Europe*', size=20, weight='bold')
fig.text(0.09, 0.853, party_description, size=11, color='#53677c')
...
r_label = f'{r:.3f}'
ax.text(0.026 if combination == 'V' else 0.974, 0.955 if combination == 'V' else 0.045, f'{n_label} districts\nPositive correlation\nPearson r = {r_label}', transform=ax.transAxes,
        va='top' if combination == 'V' else 'bottom', ha='left' if combination == 'V' else 'right', fontsize=13, linespacing=1.6, weight='bold', color='#22344b',
...
readme = f"""# {party_label} and residents born outside Europe, 2026
...
- Y: {summary["y_definition"]}, calculated from counts.
- Y-axis runs from 0 to {ymax}%, rounded up to the next 5 percentage points with at least 0.5 pp of headroom; no observations are removed. Scales differ between party combinations.
...
The CSV contains exactly the plotted points. `{STEM}-statistics.json` records definitions, exclusions, sources and the input SHA-256.

## Reproduce from the project root

```sh
.venv/bin/pip install -r reports/three-plots-2026/requirements.txt
.venv/bin/python scripts/plot_party_combinations_2026.py --combination {combination}
```
...
"""
(OUT / (STEM + '-README.md')).write_text(readme, encoding='utf-8')
print(json.dumps({'combination': party_label, 'n': len(points), 'pearson_r': r, 'y_observed_max': float(y.max()), 'y_axis_max': ymax, 'png': str(OUT / (STEM + '.png'))}, ensure_ascii=False))
