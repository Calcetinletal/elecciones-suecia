...
const name=(c:Category)=>c.name_es??c.name_sv;
...
export async function showEvolution(root:HTMLElement){
 ...
 try{
  ...
  function save(){const p=new URLSearchParams({municipality:code,origin:mode});if(mode==='countries')p.set('origins',countryCodes.join(','));history.replaceState(null,'',`${base}evolution/?${p}`);for(const a of document.querySelectorAll<HTMLAnchorElement>('nav a')){if(a.textContent==='Atlas'||a.textContent==='Países'){const url=new URL(a.href);url.searchParams.delete('county');url.searchParams.delete('district');if(code==='00')url.searchParams.delete('municipality');else url.searchParams.set('municipality',code);a.href=url.href;}}}
  function drawOrigin(){
   if(!data)return;
 ...
 try{
  ...
  async function load(){
   ...
   try{
    ...
    if(token===generation)history.replaceState(null,'',`${base}evolution/?${new URLSearchParams({scope:'district',municipality:code,year:String(year),district:id})}`);
