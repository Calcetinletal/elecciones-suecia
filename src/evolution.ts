...
function timeline(root:HTMLElement,rows:Observation[],series:Series[],breaks:number[],title:string,compact=false,initialYear?:number,breakLabels:Record<string,string>={}){
 ...
 function readout(){
  const row=rows[active];const geography=root.querySelector<HTMLElement>('.timeline-geography')!;geography.hidden=!row.scope_label;geography.textContent=row.scope_label??'';geography.title=row.scope_label??'';root.querySelector('.timeline-universe')!.textContent=row.population?`${number(row.population)} residentes · ${row.year}`:row.total?`${number(row.total)} votos válidos · ${row.year}`:row.unavailable_reason??'Sin denominador publicado';root.querySelector('.timeline-year')!.textContent=`${row.year}${row.status==='provisional'?' · provisional':row.ckm?' · CKM':''}`;
