...
export function viewTabs(state:State){
 return `<div class="view-tabs"><div class="primary-views" role="group" aria-label="Tipo de mapa">${views.map(v=>`<button type="button" class="view-choice" data-view="${v.id}" aria-pressed="${state.view===v.id}" title="${v.hint}"><svg class="view-icon" viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${v.icon}</svg><span>${v.label}</span></button>`).join('')}</div><button class="cartogram-entry" data-view="cartogram" aria-pressed="${state.view==='cartogram'}" title="Área proporcional a los votos, no al territorio">◉ Votantes</button><details class="more-maps" ${['bivariate','dominant'].includes(state.view)?'open':''}><summary>Más mapas</summary><div><button data-view="cartogram" aria-pressed="${state.view==='cartogram'}">Votos, no superficie</button><button data-view="bivariate" aria-pressed="${state.view==='bivariate'}">Cruce voto y origen</button><button data-view="dominant" aria-pressed="${state.view==='dominant'}">Ganador por origen</button></div></details></div>`;
}
