...
export function comparisonPanel(visible:District[],s:State,selected?:District){
 ...
 ${income?`<div class="comparison-inline income-inline"><span>Renta neta · ${rows[0]?.income_year} ≈</span><b>${incomeAmount(aggregateIncome(rows).mean)}</b></div>`:''}<p class="comparison-definition">Europa incluye Rusia y Turquía. Resto incluye país desconocido.</p></section>
