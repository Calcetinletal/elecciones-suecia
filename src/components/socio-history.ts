import {historyScale} from '../utils/history-scale';
...
export async function mountSocioHistory(root:HTMLElement,s:State,d?:District,areaName?:string){
 ...
 try{
  ...
  let metric='income',fromZero=false,active=0,observations:SocioPoint[]=[];
  ...
  function draw(){
   ...
   root.querySelector('.socio-definition')!.textContent=def.description;root.querySelector('.socio-universe')!.textContent=def.universe;
   ...
   const {scale:y,ticks,bottom,top,decimals}=historyScale(points.map(p=>p.v),def.unit,fromZero);
   root.dataset.yMin=String(bottom);root.dataset.yMax=String(top);
   root.querySelector('[data-socio-axis-note]')!.textContent=(fromZero?'Escala desde cero':'Escala ajustada a los datos')+' · '+(def.unit==='SEK'?'Miles de SEK/año':def.unit==='%'?'Porcentaje':'Personas');
   const x=(year:number)=>minYear===maxYear?365:76+(year-minYear)/(maxYear-minYear)*576;
   const svg=root.querySelector('svg')!;svg.setAttribute('aria-label',`${def.label} · ${scope}`);
   svg.innerHTML=`<title>${e(def.label)} · ${e(scope)}</title>${ticks.map(tick=>`<line x1="76" x2="652" y1="${y(tick)}" y2="${y(tick)}" stroke="#dfe7ec"/><text x="67" y="${y(tick)+4}" text-anchor="end">${def.unit==='SEK'?number(tick/1000,decimals)+'k':number(tick,decimals)}${def.unit==='%'?' %':''}</text>`).join('')}${[...new Set([minYear,Math.round((minYear+maxYear)/2),maxYear])].map(year=>`<text x="${x(year)}" y="225" text-anchor="middle">${year}</text>`).join('')}${points.length?`<polyline points="${points.map(p=>`${x(p.p.year)},${y(p.v)}`).join(' ')}" fill="none" stroke="${def.color}" stroke-width="3"/>${points.map(p=>`<circle data-socio-point="${p.i}" cx="${x(p.p.year)}" cy="${y(p.v)}" r="3" fill="${def.color}"><title>${p.p.year}: ${e(socioAmount(p.v,def.unit))}</title></circle>`).join('')}`:'<text x="340" y="110" text-anchor="middle">Sin observaciones disponibles</text>'}`;
  ...
  root.querySelector('[data-socio-zero]')!.addEventListener('change',event=>{const selectedYear=observations[active]?.year;fromZero=(event.target as HTMLInputElement).checked;draw();const i=observations.findIndex(p=>p.year===selectedYear);if(i>=0){active=i;readout();}});
