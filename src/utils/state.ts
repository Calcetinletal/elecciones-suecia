...
export function readState(search=location.search):State{
 ...
 if(!['municipality','district'].includes(s.cartogramUnit))s.cartogramUnit='municipality';
 if(!p.has('cartogramUnit')&&(s.municipality||s.district))s.cartogramUnit='district';
 ...
 if(s.view==='cartogram'&&s.metric.startsWith('delta_'))s.cartogramUnit='district';
