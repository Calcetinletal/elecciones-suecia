...
import {makeCartogramNodes,hasVotes,voteSlices,type CartogramNode} from '../utils/cartogram';
...
export class VoterCartogram {
 ...
 constructor(root:HTMLElement,geometry:GeoData,rows:District[],state:State,onSelect:(id:string)=>void,onMode:(mode:CartogramMode)=>void){
 ...
 update(state:State,visible:District[],national:District[]){
  const previous=this.s.cartogramMode;this.s={...state};this.national=national;this.visible=visible;
  ...
  if(key!==this.key){this.key=key;this.stopPlay();this.prepare();}else if(previous!==state.cartogramMode)this.animate();else this.draw();
  this.updateLabels();
 }
 prepare(){
  ...
  this.worldBounds=[0,0,(xmax-xmin)*scale,(ymax-ymin)*scale];this.fitBounds(this.worldBounds);
 ...
 complete(nodes:CartogramNode[]){
  ...
  if(nodes.length)this.worldBounds=[Math.min(this.worldBounds[0],...nodes.map(n=>n.x-n.r)),Math.min(this.worldBounds[1],...nodes.map(n=>n.y-n.r)),Math.max(this.worldBounds[2],...nodes.map(n=>n.x+n.r)),Math.max(this.worldBounds[3],...nodes.map(n=>n.y+n.r))];
  ...
  this.fitBounds(this.worldBounds);if(this.pendingFit){const ids=this.pendingFit;this.pendingFit=null;this.fit(ids);}this.animate();this.updateLabels();
 }
 updateLabels(){
 ...
 draw(){
  ...
  if(p<1){
   const key=[this.camera.x,this.camera.y,this.camera.k,this.canvas.width,this.canvas.height].join(',');
   if(this.geoDirty||this.geoCanvas.dataset.camera!==key){
    ...
    g.setTransform(dpr,0,0,dpr,0,0);g.translate(this.camera.x,this.camera.y);g.scale(this.camera.k,this.camera.k);
    for(const f of this.visibleShapes){g.fillStyle=this.colors.get(f.id)??'#7b8793';g.fill(f.path,'evenodd');g.strokeStyle='#ffffff';g.lineWidth=.45/this.camera.k;g.stroke(f.path);if(f.id===this.s.district){g.strokeStyle='#132d42';g.lineWidth=2/this.camera.k;g.stroke(f.path);}}
  ...
  ctx.globalAlpha=1;
  if(p>0)for(const n of this.nodes){const d=this.byId.get(n.id)!,x=n.anchorX+(n.x-n.anchorX)*p,y=n.anchorY+(n.y-n.anchorY)*p,r=n.r*Math.sqrt(p);ctx.globalAlpha=p;
 ...
 fit(ids:string[]){if(!this.ready){this.pendingFit=ids;return;}if(ids.length!==1){this.fitBounds(this.worldBounds);return;}const id=ids[0],node=this.nodes.find(n=>n.id===id),shape=this.visibleShapes.find(n=>n.id===id);if(this.s.cartogramMode!=='geography'&&node){const pad=node.r*4;this.fitBounds([node.x-pad,node.y-pad,node.x+pad,node.y+pad]);}else if(shape)this.fitBounds(shape.bounds);}
 fitBounds(bounds:number[]){if(!this.width||!this.height)return;const [x0,y0,x1,y1]=bounds,pad=24,k=Math.min((this.width-2*pad)/Math.max(x1-x0,1),(this.height-2*pad)/Math.max(y1-y0,1));this.camera={x:this.width/2-(x0+x1)/2*k,y:this.height/2-(y0+y1)/2*k,k};this.draw();this.sizeLegend();}
 resize(){const box=this.stage.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2);if(this.width===box.width&&this.height===box.height)return;this.width=box.width;this.height=box.height;this.canvas.width=Math.round(box.width*dpr);this.canvas.height=Math.round(box.height*dpr);this.fitBounds(this.worldBounds);}
 ...
 hit(px:number,py:number){const x=(px-this.camera.x)/this.camera.k,y=(py-this.camera.y)/this.camera.k;
