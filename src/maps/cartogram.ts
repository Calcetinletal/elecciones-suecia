...
import {compactCartogram,maximumOverlap,makeCartogramNodes,hasVotes,voteSlices,type CartogramNode} from '../utils/cartogram';
...
export class VoterCartogram {
 ...
 shapes:Shape[];visibleShapes:Shape[]=[];nodes:CartogramNode[]=[];byId:Map<string,District>;s:State;national:District[];visible:District[]=[];
 worker:Worker|null=null;disposed=false;key='';cache=new Map<string,CartogramNode[]>();frame=0;progress=0;pie=0;ready=false;
 baseNodes:CartogramNode[]=[];geoBounds=[0,0,1000,1000];
 ...
 constructor(root:HTMLElement,geometry:GeoData,rows:District[],state:State,onSelect:(id:string)=>void,onMode:(mode:CartogramMode)=>void,onLayout:(layout:State['cartogramLayout'])=>void){
  this.root=root;this.stage=root.querySelector('.cartogram-stage')!;this.canvas=root.querySelector('canvas')!;this.ctx=this.canvas.getContext('2d')!;
  this.s={...state};this.national=rows;this.byId=new Map(rows.map(d=>[d.district_id,d]));this.shapes=shapesFrom(geometry);this.onSelect=onSelect;this.onMode=onMode;
  ...
  root.addEventListener('click',event=>{const b=(event.target as Element).closest<HTMLButtonElement>('button');if(!b)return;
   if(b.dataset.cartogramLayout){this.stopPlay();onLayout(b.dataset.cartogramLayout as State['cartogramLayout']);}
   else if(b.dataset.cartogramMode){this.stopPlay();onMode(b.dataset.cartogramMode as CartogramMode);}
  ...
  this.canvas.addEventListener('pointerup',event=>{const moved=this.drag?.moved;this.drag=null;if(!moved){const id=this.hit(event.offsetX,event.offsetY);if(id){this.hideHover();onSelect(id);}}});
 ...
 update(state:State,visible:District[],national:District[]){
  const previous=this.s.cartogramMode,previousLayout=this.s.cartogramLayout;this.s={...state};this.national=national;this.visible=visible;
  ...
  const key=String(state.year)+':'+visible.map(d=>d.district_id).join(',');
  this.root.querySelectorAll<HTMLButtonElement>('[data-cartogram-layout]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.cartogramLayout===state.cartogramLayout)));
  if(key!==this.key||previousLayout!==state.cartogramLayout){this.key=key;this.stopPlay();this.prepare();}else if(previous!==state.cartogramMode){if(previous==='geography'||state.cartogramMode==='geography')this.fitBounds(this.boundsForMode());this.animate();}else this.draw();
  this.updateLabels();
 }
 prepare(){
  this.worker?.terminate();this.worker=null;cancelAnimationFrame(this.frame);this.ready=false;this.hideHover();this.nodes=[];this.baseNodes=[];
  ...
  this.baseNodes=nodes;this.progress=0;this.pie=0;this.draw();
  if(this.s.cartogramLayout==='compact'){this.complete(this.compactNodes());return;}
 ...
 complete(nodes:CartogramNode[]){
  ...
  if(nodes.length)this.worldBounds=[Math.min(...nodes.map(n=>n.x-n.r)),Math.min(...nodes.map(n=>n.y-n.r)),Math.max(...nodes.map(n=>n.x+n.r)),Math.max(...nodes.map(n=>n.y+n.r))];
 ...
 updateLabels(){
  ...
  (this.root.querySelector('.cartogram-layout-toggle') as HTMLElement).hidden=mode==='geography';
  this.stage.dataset.layout=this.s.cartogramLayout;
  ...
  if(mode!=='geography'&&this.s.cartogramLayout==='compact')this.root.querySelector('.cartogram-explanation')!.textContent+=' Ordenados por partido ganador; posición no geográfica.';
  this.sizeLegend();
 }
 compactNodes(){const rank=new Map(parties.map((p,i)=>[p.id,i]));return compactCartogram([...this.baseNodes].sort((a,b)=>(rank.get(this.byId.get(a.id)!.winning_party as string)??99)-(rank.get(this.byId.get(b.id)!.winning_party as string)??99)||b.r-a.r||a.id.localeCompare(b.id)),Math.max(.3,(this.width-24)/(this.height-24)));}
 boundsForMode(){return this.s.cartogramMode==='geography'?this.geoBounds:this.worldBounds;}
 status(text:string){this.root.querySelector('.cartogram-status')!.textContent=text;}
 expand(expanded:boolean){this.root.classList.toggle('cartogram-expanded',expanded);const button=this.root.querySelector('#cartogram-expand')!;button.setAttribute('aria-pressed',String(expanded));button.setAttribute('title',expanded?'Cerrar ampliación':'Ampliar gráfico');}
 ...
 draw(){
  ...
  if(p<1){
   const mapCamera=p>0?this.cameraFor(this.geoBounds):this.camera;
   ...
   ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.globalAlpha=1-p;ctx.drawImage(this.geoCanvas,0,0);ctx.restore();
  }
  ctx.globalAlpha=1;
  const geoCamera=this.cameraFor(this.geoBounds);
 ...
 fit(ids:string[]){if(!this.ready){this.pendingFit=ids;return;}if(ids.length!==1){this.fitBounds(this.boundsForMode());return;}const id=ids[0],node=this.nodes.find(n=>n.id===id),shape=this.visibleShapes.find(n=>n.id===id);if(this.s.cartogramMode!=='geography'&&node){const pad=node.r*4;this.fitBounds([node.x-pad,node.y-pad,node.x+pad,node.y+pad]);}else if(shape)this.fitBounds(shape.bounds);}
 cameraFor(bounds:number[]){const [x0,y0,x1,y1]=bounds,pad=12,k=Math.min((this.width-2*pad)/Math.max(x1-x0,1),(this.height-2*pad)/Math.max(y1-y0,1));return {x:this.width/2-(x0+x1)/2*k,y:this.height/2-(y0+y1)/2*k,k};}
 fitBounds(bounds:number[]){if(!this.width||!this.height)return;this.camera=this.cameraFor(bounds);this.draw();this.sizeLegend();}
 resize(){const box=this.stage.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2);if(this.width===box.width&&this.height===box.height)return;this.width=box.width;this.height=box.height;this.canvas.width=Math.round(box.width*dpr);this.canvas.height=Math.round(box.height*dpr);if(this.ready&&this.baseNodes.length&&this.s.cartogramLayout==='compact'){this.nodes=this.compactNodes();this.worldBounds=[0,0,Math.max(...this.nodes.map(n=>n.x+n.r)),Math.max(...this.nodes.map(n=>n.y+n.r))];}this.fitBounds(this.boundsForMode());}
 ...
 hit(px:number,py:number){if(this.stage.dataset.animating==='true')return '';const x=(px-this.camera.x)/this.camera.k,y=(py-this.camera.y)/this.camera.k;
  if(this.progress>.5&&this.ready){for(const n of this.nodes){const nx=n.anchorX+(n.x-n.anchorX)*this.progress,ny=n.anchorY+(n.y-n.anchorY)*this.progress;if(Math.hypot(x-nx,y-ny)<=n.r)return n.id;}return '';}
 ...
 hover(x:number,y:number){const id=this.hit(x,y);if(id===this.hoverId)return;this.hoverId=id;const host=this.root.querySelector<HTMLElement>('.cartogram-hover')!;this.canvas.style.cursor=id?'pointer':'grab';host.hidden=!id;if(id){const d=this.byId.get(id)!;host.innerHTML=districtHover(d,this.s.metric==='party'?'pct_'+this.s.party:this.s.metric).replace('</footer>',` · ${number(d.valid_votes)} votos válidos</footer>`);}this.draw();}
