...
export class AtlasMaps {
 currentMetric='foreign_background_pct'; maps:GLMap[]=[]; data:GeoData; byId:Map<string,District>; busy=false;disposed=false;onCamera:(v:{lng:number;lat:number;zoom:number})=>void;onSelect:(id:string)=>void;
 constructor(containers:HTMLElement[],geometry:GeoData,rows:District[],state:State,onSelect:(id:string)=>void,onCamera:(v:{lng:number;lat:number;zoom:number})=>void){
  ...
  for(const container of containers){
   ...
   map.on('idle',()=>{container.setAttribute('aria-busy','false');container.dataset.ready='true';});
   ...
   map.on('move',()=>{if(this.busy)return;this.busy=true;for(const other of this.maps)if(other!==map)other.jumpTo({center:map.getCenter(),zoom:map.getZoom(),bearing:map.getBearing(),pitch:map.getPitch()});this.busy=false;});
 ...
 fit(ids:string[]){const wanted=new Set(ids),coords:number[][]=[];const visit=(a:unknown)=>{if(Array.isArray(a)){if(typeof a[0]==='number')coords.push(a as number[]);else a.forEach(visit);}};for(const f of this.data.features)if(wanted.has(f.properties.district_id)&&'coordinates'in f.geometry)visit(f.geometry.coordinates);if(!coords.length)return;let minX=180,minY=90,maxX=-180,maxY=-90;for(const[x,y]of coords){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);}this.maps[0]?.fitBounds([[minX,minY],[maxX,maxY]] as LngLatBoundsLike,{padding:45,maxZoom:13,duration:650});}
 resize(){this.maps.forEach(m=>m.resize());}
 destroy(){this.disposed=true;this.maps.forEach(m=>m.remove());this.maps=[];}
}
