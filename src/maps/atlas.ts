import {comparisonMapState} from '../utils/comparison-maps';
...
export class AtlasMaps {
 ...
 constructor(containers:HTMLElement[],geometry:GeoData,rows:District[],state:State,onSelect:(id:string)=>void,onCamera:(v:{lng:number;lat:number;zoom:number})=>void){
  this.resizeObserver=new ResizeObserver(()=>{if(!this.disposed)this.resize();});
  ...
  for(const container of containers){
   ...
   this.maps.push(map);this.resizeObserver.observe(container);map.addControl(new maplibregl.NavigationControl({showCompass:false}),'top-right');map.addControl(new maplibregl.ScaleControl({unit:'metric'}),'bottom-left');
 ...
 update(s:State,visible:District[],national:District[]){
  ...
  this.maps.forEach((map,index)=>{
   ...
   const current:State=s.view==='compare'?comparisonMapState(s,index):s;
   const sumKey=`pct_${current.party}`;
   if(current.party.includes('+')&&this.data.features.some(f=>!(sumKey in f.properties))){
   ...
   map.setPaintProperty('districts','fill-color',color as ExpressionSpecification);map.getContainer().dataset.metric=current.metric==='party'?`pct_${current.party}`:current.metric;
  });
 }
 fit(ids:string[]){cancelAnimationFrame(this.fitFrame);const wanted=new Set(ids),coords:number[][]=[];const visit=(a:unknown)=>{if(Array.isArray(a)){if(typeof a[0]==='number')coords.push(a as number[]);else a.forEach(visit);}};for(const f of this.data.features)if(wanted.has(f.properties.district_id)&&'coordinates'in f.geometry)visit(f.geometry.coordinates);if(!coords.length)return;let minX=180,minY=90,maxX=-180,maxY=-90;for(const[x,y]of coords){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);}this.fitFrame=requestAnimationFrame(()=>{if(this.disposed)return;this.resize();const container=this.maps[0]?.getContainer(),padding=container?Math.min(35,Math.max(8,Math.min(container.clientHeight,container.clientWidth)*.08)):35;this.maps[0]?.fitBounds([[minX,minY],[maxX,maxY]] as LngLatBoundsLike,{padding,maxZoom:16,duration:650});});}
 resize(){this.maps.forEach(m=>m.resize());}
 destroy(){this.disposed=true;this.resizeObserver.disconnect();cancelAnimationFrame(this.fitFrame);this.maps.forEach(m=>m.remove());this.maps=[];}
}
